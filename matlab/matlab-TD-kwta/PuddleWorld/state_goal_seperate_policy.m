...
while (ei < maxNumEpisodes && ~convergence ), % ei<maxNumEpisodes && % ei is counter for episodes
    ...
    if mod(ei,1000)==0
        [successful_key_door_episodes, successful_key_episodes, successful_easy_episodes, successful_basic_key_door, scores_vec,scores_vec_basic,score_key,score_easy, total_episodes] = test_func_seperate_policy(W_ih_big, b_ih_big, W_ho_big, b_ho_big);        
...
%     g = neighbor_state(s0,xVector,yVector,radius);
    ...
    if mod(ei,10000) == 0
        file_name = 'all_variables_April_18th.mat';
        save(file_name) 
    end
%     if mod(ei,1000)==0    
